...
class ErrandRecognitionTestBench(CustomRecognition):
    def analyze(
        ...
        # Done init
        time1 = datetime.now()
        reco_detail = new_context.run_recognition("TempErrandRecoTestBench001", images[0], 
                                                  {"TempErrandRecoTestBench001": {"recognition": "custom", "custom_recognition": "ErrandRecoSingle", "roi": dims.box_t_1st.roi}})
        print(reco_detail)
        print(datetime.now() - time1)
        errand = jsons.load(reco_detail.best_result.detail, Errand)
        print(errand)
...
class ErrandRecognitionSingle(CustomRecognition):
    def analyze(
        ...
        new_context = context.clone()
        errand = Errand()
        reco_detail_out_now = new_context.run_recognition("Errand_Base_Find_In_Progress_Mark", argv.image, 
                                                          {"Errand_Base_Find_In_Progress_Mark": {"roi": argv.roi.roi}})
        reco_detail_done = new_context.run_recognition("Errand_Base_Find_Done_Mark", argv.image, 
                                                       {"Errand_Base_Find_Done_Mark": {"roi": argv.roi.roi}})
        errand.state = ErrandState.OUT_NOW if reco_detail_out_now is not None else ErrandState.DONE if reco_detail_done is not None else ErrandState.DEFAULT
        reco_detail_sc = new_context.run_recognition("Errand_Base_Find_Sealed_Crystal", argv.image, 
                                                     {"Errand_Base_Find_Sealed_Crystal": {"roi": argv.roi.roi, "roi_offset": (0, 0, 0, 0)}})
        reco_detail_gc = new_context.run_recognition("Errand_Base_Find_God_Crystal", argv.image, 
                                                     {"Errand_Base_Find_God_Crystal": {"roi": argv.roi.roi, "roi_offset": (0, 0, 0, 0)}})
        errand.reward_type = ErrandReward.SC if reco_detail_sc is not None else ErrandReward.GC if reco_detail_gc is not None else ErrandReward.OTHERS
        errand.reward_count = len(reco_detail_sc.filterd_results) if reco_detail_sc is not None else len(reco_detail_gc.filterd_results) if reco_detail_gc is not None else 0
        reco_detail_times = new_context.run_recognition("Errand_Base_Find_Times", argv.image, 
                                                        {"Errand_Base_Find_Times": {"roi": argv.roi.roi, "roi_offset": (0, 0, 0, 0)}})
        box_time_cost = argv.roi + dims.relative_box_negative_whole_errand + dims.relative_box_time_cost
        box_time_expire = argv.roi + dims.relative_box_negative_whole_errand + dims.relative_box_time_expire
        text_time_cost_tp = tuple(r.text for r in reco_detail_times.filterd_results if is_in_rect(box_time_cost)(Rect(*r.box))) if reco_detail_times is not None else tuple()
        text_time_cost = text_time_cost_tp[0] if len(text_time_cost_tp) == 1 else None
        errand.time_cost = datetime.strptime(regex_time.search(text_time_cost).group(), "%H:%M:%S") - datetime.strptime("0", "%S") if text_time_cost is not None else None
        text_time_expire_tp = tuple(r.text for r in reco_detail_times.filterd_results if is_in_rect(box_time_expire)(Rect(*r.box))) if reco_detail_times is not None else tuple()
        text_time_expire = text_time_expire_tp[0] if len(text_time_expire_tp) == 1 else None
        errand.time_expire = datetime.strptime(regex_time.search(text_time_expire).group(), "%H:%M:%S") - datetime.strptime("0", "%S") if text_time_expire is not None else None
...
class ErrandRecognition1(CustomRecognition):
    ...
    def analyze(
        ...
        if count_limited_errands <= 3:
            ...
            reco_detail = new_context.run_recognition("Errand_Base_Flag_Seen_Limited_Errand_At_Slot_1", images[2], 
        ...
        if count_limited_errands > 0:
            ...
            reco_detail_out_now = new_context.run_recognition("Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_1", images[2], 
                                                {"Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_1": {"roi_offset": roi_offset.roi}})
            reco_detail_sealed_crystal = new_context.run_recognition("Errand_Base_Find_Sealed_Crystal", images[2], 
                                                {"Errand_Base_Find_Sealed_Crystal": {"roi": dims.box_t_1st.roi, "roi_offset": roi_offset.roi}})
            reco_detail_god_crystal = new_context.run_recognition("Errand_Base_Find_God_Crystal", images[2], 
                                                {"Errand_Base_Find_God_Crystal": {"roi": dims.box_t_1st.roi, "roi_offset": roi_offset.roi}})
            reco_detail_times = new_context.run_recognition("Errand_Base_Find_Times", images[2],
        ...
        if count_limited_errands in range(4,7):
            ...
            reco_detail_out_now = new_context.run_recognition("Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_-1", images[3], 
                                                {"Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_-1": {"roi_offset": roi_offset.roi}})
            reco_detail_sealed_crystal = new_context.run_recognition("Errand_Base_Find_Sealed_Crystal", images[3], 
                                                {"Errand_Base_Find_Sealed_Crystal": {"roi": dims.box_b_1st.roi, "roi_offset": roi_offset.roi}})
            reco_detail_god_crystal = new_context.run_recognition("Errand_Base_Find_God_Crystal", images[3], 
                                                {"Errand_Base_Find_God_Crystal": {"roi": dims.box_b_1st.roi, "roi_offset": roi_offset.roi}})
            reco_detail_times = new_context.run_recognition("Errand_Base_Find_Times", images[3],
        ...
        num_errands = 3
        roi_offset = sum((dims.offset_grow_one_down,) * (num_errands - 1), Rect()) 
        reco_detail_out_now = new_context.run_recognition("Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_1", images[0], 
                                            {"Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_1": {"roi_offset": roi_offset.roi}})
        reco_detail_sealed_crystal = new_context.run_recognition("Errand_Base_Find_Sealed_Crystal", images[0], 
                                            {"Errand_Base_Find_Sealed_Crystal": {"roi": dims.box_t_1st.roi, "roi_offset": roi_offset.roi}})
        reco_detail_god_crystal = new_context.run_recognition("Errand_Base_Find_God_Crystal", images[0], 
                                            {"Errand_Base_Find_God_Crystal": {"roi": dims.box_t_1st.roi, "roi_offset": roi_offset.roi}})
        reco_detail_times = new_context.run_recognition("Errand_Base_Find_Times", images[0],
                                            {"Errand_Base_Find_Times": {"roi": dims.box_t_1st.roi, "roi_offset": roi_offset.roi}})
        for i in range(num_errands):
        ...
        roi_offset = sum((dims.offset_grow_one_up,) * (num_errands - 1), Rect()) 
        reco_detail_out_now = new_context.run_recognition("Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_-1", images[1], 
                                            {"Errand_Base_Flag_Seen_In_Progress_Mark_At_Slot_-1": {"roi_offset": roi_offset.roi}})
        reco_detail_sealed_crystal = new_context.run_recognition("Errand_Base_Find_Sealed_Crystal", images[1], 
                                            {"Errand_Base_Find_Sealed_Crystal": {"roi": dims.box_b_1st.roi, "roi_offset": roi_offset.roi}})
        reco_detail_god_crystal = new_context.run_recognition("Errand_Base_Find_God_Crystal", images[1], 
                                            {"Errand_Base_Find_God_Crystal": {"roi": dims.box_b_1st.roi, "roi_offset": roi_offset.roi}})
        reco_detail_times = new_context.run_recognition("Errand_Base_Find_Times", images[1],
                                            {"Errand_Base_Find_Times": {"roi": dims.box_b_1st.roi, "roi_offset": roi_offset.roi}})
        for i in range(num_errands):
...
class ErrandRecognition(CustomRecognition):
    ...
    def analyze(
        ...
        new_context = context.clone()
        # Get 1st Image
        img = new_context.tasker.controller.post_screencap().wait().get()
        images = images + (img,); reco_times = reco_times + (datetime.now(),)
        # check if at Errand Page, Daily Tab, Top Position
        reco_detail = new_context.run_recognition("Errand_Base_Flag_At_Errand_Page_0", img)
        if reco_detail is None: logger.debug("Not at Errand Page with 0 Finished Errands, Stop Recognition"); return failed_result
        reco_detail = new_context.run_recognition("Errand_Base_Flag_At_Daily_Tab", img)
        if reco_detail is None: logger.debug("Not at Daily Tab"); return failed_result
        reco_detail = new_context.run_recognition("Errand_Base_Flag_Seen_Scroll_Bar_Top_5", img) # Assume Daily Tab always have 5 Errands
        ...
        action_result = new_context.run_action("Errand_Base_Run_VerticalSwipe", dims.box_t_4th + dims.offset_click, "", 
        ...
        img = new_context.tasker.controller.post_screencap().wait().get()
        images = images + (img,); reco_times = reco_times + (datetime.now(),)
        reco_detail = new_context.run_recognition("Errand_Base_Flag_Seen_Scroll_Bar_Bottom_68px", img)
        ...
        action_result = new_context.run_action("Errand_Base_Run_VerticalSwipe", dims.box_b_4th + dims.offset_click, "", 
                                           {"Errand_Base_Run_VerticalSwipe":{"custom_action_param": {"delta_y": delta_y}}})
        if action_result is None: logger.debug("Failed to Scroll Up"); return failed_result
        # Go to Limited Tab
        action_result = new_context.run_action("Errand_Base_Tap_Limited_Tab")
        ...
        reco_detail = new_context.run_recognition("Errand_Base_Flag_At_Limited_Tab", img)
        ...
        reco_detail = new_context.run_recognition("Errand_Base_Flag_Seen_Scroll_Bar_Top_456", img)
        if reco_detail is None: logger.debug("No Need to Scroll Down at Limited Tab"); count_limited_errands = 3
        else:
            for i in range(4, 7):
                reco_detail = new_context.run_recognition(f"Errand_Base_Flag_Seen_Scroll_Bar_Top_{i}", img)
